...
export function AppointmentDetailModal({ 
  ...
  const [selectedMenuIds, setSelectedMenuIds] = useState<number[]>(
    (appointment.services as { menuIds: number[] })?.menuIds || []
  )
  ...
  // 予約更新ハンドラー
  const handleUpdateAppointment = async (formData: FormData) => {
    setLoading(true)
    setError(null)
  ...
  // 施術完了ハンドラー
  const handleCompleteTreatment = async (formData: FormData) => {
    setLoading(true)
    setError(null)
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error('施術完了の記録に失敗しました')
      }
                ...
                <div className="text-right">
                  ...
                  <span className="text-lg font-medium text-gray-800">
                    ¥{selectedMenuIds.reduce((sum, id) => 
                      sum + (TREATMENT_MENU.find(menu => menu.id === id)?.price || 0), 
                      0
                    ).toLocaleString()}
                  </span>
                </div>
                ...
                <div className="bg-white rounded-lg w-full max-w-2xl max-h-[90vh] overflow-y-auto">
                  <div className="p-6">
                    ...
                    <form onSubmit={(e) => {
                      e.preventDefault();
                      const formData = new FormData(e.currentTarget);
                      handleCompleteTreatment(formData);
                    }} className="space-y-4">
                    <div>
